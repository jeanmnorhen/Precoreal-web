    ...
    var AuthService = class {
        ...
        async register(dto) {
            const existing = await this.db
                .select()
                .from(shared_1.usuarios)
                .where((0, drizzle_orm_1.eq)(shared_1.usuarios.email, dto.email))
                .limit(1);
        ...
        async me(userId) {
            const [user] = await this.db
                .select()
                .from(shared_1.usuarios)
                .where((0, drizzle_orm_1.eq)(shared_1.usuarios.id, userId))
                .limit(1);
            if (!user)
                return null;
            return {
