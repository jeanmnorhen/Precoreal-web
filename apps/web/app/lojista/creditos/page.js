...
exports.default = LojistaCreditos;
...
function LojistaCreditos() {
    ...
    const comprar = async (valorCentavos) => {
        setCarregando(true);
        try {
            const res = await api_1.api.lojista.comprarCreditos(valorCentavos, '');
    ...
    return (<div className="animate-fade-in-up">
      ...
      {!clientSecret ? (<div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
          {VALORES.map((v) => (<button key={v.centavos} onClick={() => comprar(v.centavos)} disabled={carregando} className="p-6 rounded-xl text-center transition-all hover:shadow-md hover:-translate-y-0.5 disabled:opacity-50" style={{ border: '1.5px solid var(--color-navy-200)', background: 'var(--color-card)' }}>
        ...
        </div>) : (<div className="p-6 rounded-xl text-center animate-scale-in" style={{ border: '1px solid var(--color-border)', background: 'var(--color-card)' }}>
          ...
          <button onClick={() => setClientSecret(null)} className="mt-6 px-6 py-2.5 rounded-lg font-medium transition-opacity hover:opacity-70" style={{ border: '1px solid var(--color-border)' }}>
